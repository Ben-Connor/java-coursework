...
def test_create_targets(client: TestClient, session: BegunSession) -> None:
    ...
    for idx, target_data in enumerate(targets_data):
        ...
        target = NutrientTargetTable.model_validate(response.json()["target"])
        assert session.scalar(select(NutrientTargetTable).where(NutrientTargetTable.id == BULKING_NUTRIENT_TARGETS[idx].id)) == target
...
def test_get_target_by_id(client: TestClient, session: BegunSession) -> None:
    ...
    target = NutrientTargetTable.model_validate(response.json()["target"])
    assert session.scalar(select(NutrientTargetTable).where(NutrientTargetTable.id == BULKING_NUTRIENT_TARGET_001.id)) == target
...
def test_get_all_targets(client: TestClient, session: BegunSession) -> None:
    ...
    targets = [NutrientTargetTable.model_validate(item) for item in response_002.json()["targets"]]
    assert list(session.scalars(select(NutrientTargetTable))) == targets

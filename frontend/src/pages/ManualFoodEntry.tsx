...
const ManualFoodEntry: React.FC = () => {
  ...
  const [vitamins, setVitamins] = useState<{ name: string; amount: string; unit: string }[]>([]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const foodData = {
      ...
      fat: parseFloat(fat),
      vitamins: vitamins.map(vit => ({ ...vit, amount: parseFloat(vit.amount) })),
    };
  ...
  const handleVitaminChange = (index: number, field: keyof (typeof vitamins)[0], value: string) => {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {vitamins.map((vit, index) => (
          <div key={index}>
            ...
            <input
              ...
              placeholder="Amount"
              value={vit.amount}
              onChange={(e) => handleVitaminChange(index, "amount", e.target.value)}
              required
            />
            <select
              value={vit.unit}
        ...
        <button type="button" onClick={handleAddVitamin}>Add Vitamin</button>
